...
- 研究者身份：作者既是“写作者”也是“研究者”，通过长期写作与回看形成自反式观察。
- 研究对象：写作中的“自我—他者—情境”三元关系（人物、事件、地点与时代语境）。
- 方法组件：
  - 田野札记/研究备忘：在生成年度与主题报告时，同步记录研究者的即时观察与疑问（`notes/research_memos/`）。
  - 概念提炼：为反复出现的核心观念建立“概念卡片”（`notes/concepts/*.md`），链接到出处段落。
  - 关系图谱：用共现/时间线构建“人物—事件—地点”网络（阶段 2 产出），配合研究札记进行解释性分析。
  - 反思回合：每个阶段结束生成“自反式评论”（`reports/reflection_<stage>.md`），记录选择、偏好与局限。
- 伦理与边界：
  - 优先保护具体他人隐私；在公开分享时对敏感细节进行去标识化与模糊化；
  - 以“兴趣驱动与自我成长”为核心，不追求外部评审标准的“客观完备”。

---

## 兴趣导向的任务编排原则

1) 兴趣优先：同等价值项中优先选择当前最想做、最有好奇心的任务；
2) 微增量交付：将任务切成 1–3 小时可完成的小目标，随做随见成果；
3) 可逆与可复现：任何分析都能被脚本重放，确保随时暂停/切换而不丢进度；
4) 解释优先于指标：在报告中明确“为什么值得关心”，而不仅是“数值变化”；
5) 以写作反哺研究：阶段性结果转化为短文或卡片，促进后续问题生成。
